    ...
    try {
      ...
      if (auth?.user?._id) {
        let record = await axios.get(
          `http://localhost:8000/carts/usercart?userId=${auth?.user?._id}`,
          { headers }
        );
  ...
  const deleteCart = async (id) => {
    try {
      ...
      let { data } = await axios.delete(
        `http://localhost:8000/carts/deletecart?id=${id}`,
        { headers }
      );
      if (data.success) {
  ...
  const editCart = async (id, qty) => {
    ...
    try {
      ...
      let { data } = await axios.put(
        `http://localhost:8000/carts/editcart?id=${id}`,
        postData,
        { headers }
      );
      if (data.success) {
        ...
        <div className="row">
          ...
          <div className="col-lg-7">
            ...
            >
              ...
              <div className="card-body">
                <table className="table table-striped table-hove table-dark">
                          ...
                          <tr key={cart._id}>
                            ...
                            <td>
                              <input
                                type="number"
                                onChange={(e) =>
                                  editCart(cart._id, e.target.value)
                                }
                                value={cart.qty}
